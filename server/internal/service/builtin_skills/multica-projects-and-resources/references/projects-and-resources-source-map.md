...
- `git_repo` is the canonical git repo resource type; `github_repo` is a backward-compatible alias. `isGitRepoResourceType` (in `server/internal/handler/project_resource.go` and `server/cmd/multica/cmd_project.go`) treats both identically at every dispatch point; no data is normalized, so existing `github_repo` rows are untouched.
- `project resource add` supports shortcuts for `git_repo`/`github_repo` (`--url`, non-JSON `--ref` for checkout ref, `--default-branch-hint`) and `local_directory` (`--local-path`, `--daemon-id`, `--ref-label`), or generic JSON `--ref '<json>'`.
- `project resource update` merges shortcut edits with existing `resource_ref` so a partial edit does not clobber required fields; non-JSON `--ref` updates the git repo `resource_ref.ref`.
